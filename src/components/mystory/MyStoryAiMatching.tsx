...
const MATCHED_RECORDS = [
  ...
  },
  {
    ...
    score: 96,
    steps: [
      ['상황', '사용자 이탈률이 높아 서비스 개선 방향을 정하기 위한 팀 회의를 진행했습니다.'],
      ['대응', '사용자 피드백을 유형별로 분류하고 영향도와 구현 난이도를 기준으로 우선순위를 제안했습니다.'],
      ['소통', '기획과 개발 관점의 차이를 조율하며 핵심 개선안을 모두가 이해할 수 있도록 공유했습니다.'],
      ['결과', '합의된 개선안을 반영한 뒤 핵심 화면의 이탈률을 낮추고 다음 실험 계획까지 수립했습니다.'],
    ],
  },
  {
    id: 3,
    ...
    score: 92,
    steps: [
      ['상황', '교내 프로젝트에서 설문과 이용 로그를 함께 분석해 사용자 행동의 원인을 설명해야 했습니다.'],
      ['대응', '데이터를 정제한 뒤 주요 지표를 정의하고 행동 패턴별로 사용자 그룹을 나누어 비교했습니다.'],
      ['소통', '분석 결과를 차트와 핵심 문장으로 정리해 비전공 팀원도 이해할 수 있도록 발표했습니다.'],
      ['결과', '데이터에 근거한 개선안이 최종 제안에 반영되었고 프로젝트 평가에서 높은 점수를 받았습니다.'],
    ],
  },
];
    ...
    <article className="rounded-2xl border border-grey-100 bg-white p-5">
      ...
      {expanded && (
        <div className="mt-5 grid gap-3">
          {record.steps.map(([label, text], index) => (
