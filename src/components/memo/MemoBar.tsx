...
export const MemoBar = ({ count, onCancel, onMove, onDelete, isDeleting = false }: MemoBarProps) => (
  <div className="absolute left-1/2 top-[18px] z-20 flex h-14 w-[660px] max-w-[60%] -translate-x-1/2 items-center rounded-2xl border border-grey-100 bg-white pl-6 pr-8 text-grey-900 shadow-[0_0_30px_rgba(22,53,164,0.08)]">
    ...
    <button type="button" onClick={onMove} className="ml-auto flex cursor-pointer items-center gap-2 text-body2-md text-grey-900">
    ...
    <button
      ...
      disabled={isDeleting}
      onClick={onDelete}
      className="ml-8 flex cursor-pointer items-center gap-2 text-body2-md text-grey-900 disabled:cursor-not-allowed disabled:opacity-50"
    >
